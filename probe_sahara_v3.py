...
def main():
    ...
    try:
        ...
        print(f"Chip V3 raw     : {chip_v3.hex()}")

        if len(chip_v3) >= 44:
            chip_identifier = int.from_bytes(chip_v3[0:4], "little")
            msm_id = int.from_bytes(chip_v3[36:40], "little")
            oem_id = int.from_bytes(chip_v3[40:42], "little")
            model_id = int.from_bytes(chip_v3[42:44], "little")

            if oem_id == 0 and len(chip_v3) >= 46:
                oem_id = int.from_bytes(chip_v3[44:46], "little")

            hwid = f"00{msm_id:06x}{oem_id:04x}{model_id:04x}"

            print(f"Chip Identifier : 0x{chip_identifier:08x}")
            print(f"MSM ID          : 0x{msm_id:06x}")
            print(f"OEM ID          : 0x{oem_id:04x}")
            print(f"Model ID        : 0x{model_id:04x}")
            print(f"Constructed HWID: 0x{hwid}")
        else:
            print(
                f"WARNING: V3 chip response shorter than expected: "
                f"{len(chip_v3)} bytes"
            )

        print("OEM PKHASH      : not queried; Sahara v3 may restrict cmd 0x03")
